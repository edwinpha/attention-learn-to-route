...
import os
import pickle
from problems.pctsp.state_pctsp_3d import StatePCTSP3D
from utils.beam_search import beam_search


class PCTSP3D(object):

    NAME = 'pctsp_3d'  # Prize Collecting TSP, without depot, with penalties
    ...
    def make_dataset(*args, **kwargs):
        return PCTSP3DDataset(*args, **kwargs)
...
class PCTSP3D_Det(PCTSP3D):

    @staticmethod
    def get_costs(dataset, pi):
        return PCTSP3D._get_costs(dataset, pi, stochastic=False)

    @staticmethod
    def make_state(*args, **kwargs):
        return StatePCTSP3D.initialize(*args, **kwargs, stochastic=False)


class PCTSP3D_Stoch(PCTSP3D):

    # Stochastic variant of PCTSP, the real (stochastic) prize is only revealed when node is visited

    @staticmethod
    def get_costs(dataset, pi):
        return PCTSP3D._get_costs(dataset, pi, stochastic=True)

    @staticmethod
    def make_state(*args, **kwargs):
        return StatePCTSP3D.initialize(*args, **kwargs, stochastic=True)


def generate_instance(size, penalty_factor=3):
    depot = torch.rand(3)
    loc = torch.rand(size, 3)
...
class PCTSP3DDataset(Dataset):
    
    def __init__(self, filename=None, size=50, num_samples=1000000, offset=0, distribution=None):
        super(PCTSP3DDataset, self).__init__()
